...
CREATE OR REPLACE FUNCTION preprojet.modifierMontantOperation(nom_source VARCHAR(100), prenom_source VARCHAR(100),
    ...
    date_operation TIMESTAMP, montant_operation INTEGER) RETURNS RECORD AS $$
    DECLARE
        ret RECORD;
    BEGIN
...
SELECT * FROM preprojet.modifierMontantOperation(VARCHAR(100) 'Damas',VARCHAR(100) 'Christophe',
    ...
    t(id int, src character(9), dest character(9), montant int, date timestamp);
-- Question 6 : ok
select * from preprojet.operations o where o.id_operation = 1;

-- QUESTION 7
CREATE OR REPLACE FUNCTION preprojet.supprimerOperation(nom_source VARCHAR(100), prenom_source VARCHAR(100),
    cpt_source CHARACTER(9), nom_destination VARCHAR(100), prenom_destination VARCHAR(100), cpt_destination CHARACTER(9),
    date_operation TIMESTAMP, montant_operation INTEGER) RETURNS BOOLEAN AS $$
    DECLARE
        ret BOOLEAN:=false;
        record RECORD;
    BEGIN
        FOR record IN SELECT * FROM preprojet.operations LOOP
...
$$ LANGUAGE plpgsql;
-- Question 7
--select * from preprojet.supprimerOperation(VARCHAR(100) 'Damas',VARCHAR(100) 'Christophe',
--    CHARACTER(9) '123456789',VARCHAR(100) 'Cambron',VARCHAR(100) 'Isabelle',
--    CHARACTER(9) '563212564', TIMESTAMP '2006-12-01', 20);
-- Question 7 : ok
...
CREATE OR REPLACE FUNCTION preprojet.balance(compte CHARACTER(9)) RETURNS SETOF RECORD AS $$
    DECLARE
        ...
        balance INTEGER:=0;
    BEGIN
        FOR record IN SELECT o.date_op, o.compte_source, o.compte_destination, o.montant FROM preprojet.operations o LOOP
...
$$ LANGUAGE plpgsql;
-- Question 8 : ok
select * from preprojet.balance('563212564') t(date timestamp, tiers character(9), montant integer);
